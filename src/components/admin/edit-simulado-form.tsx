...
interface EditSimuladoFormProps {
  simuladoId: string
  onSuccess?: () => void
}

export function EditSimuladoForm({ simuladoId, onSuccess }: EditSimuladoFormProps) {
    ...
    const loadSimulado = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('simulados')
          .select('id, title, description, duration_minutes, fator_correcao, allow_retake, start_date, end_date, show_ranking')
          .eq('id', simuladoId)
          .single()
        ...
        if (data) {
          ...
          setFatorCorrecao(data.fator_correcao ? data.fator_correcao.toString() : '')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error('Erro inesperado ao atualizar simulado')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Edite os dados do simulado</CardDescription>
        </CardHeader>
